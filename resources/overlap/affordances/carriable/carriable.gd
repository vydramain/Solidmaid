...
func on_picked_up(_slots: CarrySlots, _slot_name: String) -> void:
	...
	if host == null:
		return
	cache_collision(host)
	host.freeze = true
	host.sleeping = true
	host.linear_velocity = Vector3.ZERO
	host.angular_velocity = Vector3.ZERO
	disable_collision(host)
	call_host_hook(host, "on_carriable_picked_up", [_slots, _slot_name])
...
func on_released(release_velocity: Vector3) -> void:
	...
	host.freeze = false
	host.sleeping = false
	restore_collision(host)
	host.linear_velocity = release_velocity
	host.angular_velocity = Vector3.ZERO
	call_host_hook(host, "on_carriable_released", [release_velocity])


func get_host() -> RigidBody3D:
...
func cache_collision(host: RigidBody3D) -> void:
	_cached_layer = host.collision_layer
	_cached_mask = host.collision_mask


func disable_collision(host: RigidBody3D) -> void:
	host.collision_layer = 0
	host.collision_mask = 0


func restore_collision(host: RigidBody3D) -> void:
	if _cached_layer >= 0:
...
func call_host_hook(host: Node, method: StringName, args: Array) -> void:
