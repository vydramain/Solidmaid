...
func queue_rebuild() -> void:
	...
	if _queued:
		return
	_queued = true
	call_deferred("rebuild_animation_deferred")


func rebuild_animation_deferred() -> void:
	_queued = false
	rebuild_animation()


func rebuild_animation() -> void:
	var root := get_animation_root()
	if root == null:
	...
	var library := get_or_create_library()
	var swing_animation := get_or_create_animation(library, animation_name)
	var reset_animation := get_or_create_animation(library, reset_animation_name)

	var swing_keys := build_keys(target)
	fill_swing_animation(swing_animation, swing_keys)
	fill_reset_animation(reset_animation, swing_keys)


func get_animation_root() -> Node:
	if get_owner():
		return get_owner()
	return get_parent()


func get_or_create_library() -> AnimationLibrary:
...
func get_or_create_animation(library: AnimationLibrary, name: StringName) -> Animation:
...
func build_keys(target: Node3D) -> Array:
	...
	var windup_positions := sample_linear_phase(
		base_position,
		base_position + start_offset,
		t_windup,
		windup_samples
	)
	var windup_yaws := sample_linear_rotation(yaw_idle, yaw_start, windup_samples)
	append_phase_keys(keys, windup_positions, windup_yaws, t_cursor, base_rotation)
	t_cursor += t_windup

	# Phase 2: swing (start -> apex -> end)
	var swing_positions := sample_quadratic_bezier_phase(
		base_position + start_offset,
	...
	var swing_yaws := sample_eased_rotation(yaw_start, yaw_end, swing_samples)
	append_phase_keys(keys, swing_positions, swing_yaws, t_cursor, base_rotation, true)
	t_cursor += t_swing

	# Phase 3: follow-through (end -> follow)
	var follow_positions := sample_linear_phase(
		base_position + end_offset,
		base_position + follow_offset,
		t_follow,
		follow_samples
	)
	var follow_yaws := sample_linear_rotation(yaw_end, yaw_end, follow_samples)
	append_phase_keys(keys, follow_positions, follow_yaws, t_cursor, base_rotation, true)
	t_cursor += t_follow

	# Phase 4: recovery (follow -> idle)
	var recovery_positions := sample_linear_phase(
		base_position + follow_offset,
		base_position,
		t_recovery,
		recovery_samples
	)
	var recovery_yaws := sample_linear_rotation(yaw_end, yaw_idle, recovery_samples)
	append_phase_keys(keys, recovery_positions, recovery_yaws, t_cursor, base_rotation, true)

	return keys


func fill_swing_animation(animation: Animation, keys: Array) -> void:
	clear_tracks(animation)
...
func fill_reset_animation(animation: Animation, keys: Array) -> void:
	clear_tracks(animation)
...
func clear_tracks(animation: Animation) -> void:
	for track_index in range(animation.get_track_count() - 1, -1, -1):
		animation.remove_track(track_index)


func quadratic_bezier(a: Vector3, b: Vector3, c: Vector3, t: float) -> Vector3:
	var omt := 1.0 - t
...
func sample_linear_phase(a: Vector3, b: Vector3, duration: float, count: int) -> Array:
...
func sample_quadratic_bezier_phase(a: Vector3, b: Vector3, c: Vector3, duration: float, count: int) -> Array:
	...
	for i in range(samples_count):
		...
		result.append({
			"time": t * duration,
			"position": quadratic_bezier(a, b, c, t),
		})
	return result


func sample_linear_rotation(a: float, b: float, count: int) -> Array:
	var samples_count = max(2, count)
	var result: Array = []
	for i in range(samples_count):
		var t := float(i) / float(samples_count - 1)
...
func sample_eased_rotation(a: float, b: float, count: int) -> Array:
	var samples_count = max(3, count)
	var result: Array = []
	for i in range(samples_count):
		var t := float(i) / float(samples_count - 1)
...
func append_phase_keys(keys: Array, positions: Array, yaws: Array, start_time: float, base_rotation: Vector3, skip_first := false) -> void:
