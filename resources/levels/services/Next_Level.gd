...
func _ready() -> void:
	Custom_Logger.log(self, "NextLevel trigger initialized on scene: " + str(get_tree().current_scene.scene_file_path))
...
func transfer_player_to_next_level() -> void:
	...
	Custom_Logger.log(self, "Current level detected: " + scene_file)
