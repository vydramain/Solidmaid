...
- На входе остаются только реальные кнопки рук: `hand_right` (LMB / правый триггер) и `hand_left` (RMB / левый триггер). Никаких отдельных `interact`/`attack`/`throw` действий — выбор поведения решает связка ability + affordance.
...
1. Игрок жмёт кнопку руки (LMB/RT или RMB/LT). Контроллер знает, какая рука должна среагировать.
